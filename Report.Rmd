...
#extract_high_corr(corr_data_class2, 0.95)
```
...
final_model1_base <- lm(WinnerRed ~ B_avg_KD + B_avg_opp_SIG_STR_pct + B_avg_TD_att + B_avg_opp_TD_att + B_avg_opp_HEAD_landed + B_avg_CLINCH_att + B_win_by_Decision_Majority + B_win_by_Decision_Unanimous + B_win_by_Submission + R_avg_KD + R_avg_opp_SIG_STR_pct + R_avg_opp_TD_pct + R_avg_opp_SUB_ATT + R_avg_REV + R_avg_SIG_STR_landed + R_avg_BODY_landed + R_avg_LEG_att + R_avg_opp_CLINCH_att + R_current_win_streak + B_age + R_age + B_StanceSwitch, family = "binomial", data = class1_train)

final_model1_interactions <- lm(WinnerRed ~ B_avg_KD + B_avg_opp_SIG_STR_pct + B_avg_TD_att + B_avg_opp_TD_att + B_avg_opp_HEAD_landed + B_avg_CLINCH_att + B_win_by_Decision_Majority + B_win_by_Decision_Unanimous + B_win_by_Submission + R_avg_KD + R_avg_opp_SIG_STR_pct + R_avg_opp_TD_pct + R_avg_opp_SUB_ATT + R_avg_REV + poly(R_avg_SIG_STR_landed, 2) + R_avg_BODY_landed + R_avg_LEG_att + R_avg_opp_CLINCH_att + R_current_win_streak + B_age + R_age + B_StanceSwitch + R_avg_REV*R_avg_SIG_STR_landed + R_avg_BODY_landed*R_avg_SIG_STR_landed + R_avg_opp_TD_pct*R_avg_opp_SIG_STR_pct, family = "binomial", data = class1_train)

anova(final_model1_base, final_model1_interactions)
...
full_logistic_model_4 <- glm(WinnerRed ~ ., data = class4_train, family = "binomial")


full_predictions4 <- predict(full_logistic_model_4, newdata = class4_test, type = "response")
full_predicted_classes4 <- ifelse(full_predictions4 > 0.5, 1, 0)
```

```{r}
conf_matrix4 <- confusionMatrix(as.factor(full_predicted_classes4), as.factor(class4_test$WinnerRed))
accuracy4 <- conf_matrix4$overall['Accuracy']
precision4 <- conf_matrix4$byClass['Precision']
recall4 <- conf_matrix4$byClass['Recall']
F1_Score4 <- conf_matrix4$byClass['F1']
